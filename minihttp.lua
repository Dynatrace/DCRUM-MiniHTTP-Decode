...
package.cpath = package.cpath .. ';./rtmapi/?.so'
require 'amd'

local HTTP_COMMANDS = {'GET', 'POST', 'PUT', 'DELETE', 'CONNECT', 'HEAD'}
local HTTP_PROPERTY = {HOST = 'Host:', USER_AGENT = 'User-Agent:',
	AUTHORIZATION = 'Authorization:', PROXY_AUTHORIZATION = 'Proxy-Authorization:'}
...
local function parse_authorization(pstr, stats)
	local base64_string = pstr:match('Basic (.-)$')
	if base64_string then
		local decoded_string = amd.fromBase64(base64_string)
		if decoded_string then
			local user_name = decoded_string:match('(.-):')
			if user_name then
				stats:setUserName(user_name)
			end
		end
	end
end
...
function parse_request(payload, stats)
	...
	if properties[HTTP_PROPERTY.AUTHORIZATION] then
		parse_authorization(properties[HTTP_PROPERTY.AUTHORIZATION], stats)
	elseif properties[HTTP_PROPERTY.PROXY_AUTHORIZATION] then
		parse_authorization(properties[HTTP_PROPERTY.PROXY_AUTHORIZATION], stats)
	end
	if host ~= nil then
		host = trim(host)
